...
class DetectorBackboneWithFPN(nn.Module):
    ...
    def __init__(self, out_channels: int):
        ...
        print(f"Dummy ouput shape: {dummy_out_shapes}")
        ...
        self.fpn_params = nn.ModuleDict({
            'l3': nn.Conv2d(dummy_out_shapes[0][1][1], out_channels ,kernel_size=1, stride=1, padding=0),
            'l4': nn.Conv2d(dummy_out_shapes[1][1][1], out_channels ,kernel_size=1, stride=1, padding=0),
            'l5': nn.Conv2d(dummy_out_shapes[2][1][1], out_channels ,kernel_size=1, stride=1, padding=0),
            'p3': nn.Conv2d(self.out_channels, self.out_channels,kernel_size=3, stride=1, padding=1),
            'p4': nn.Conv2d(self.out_channels, self.out_channels,kernel_size=3, stride=1, padding=1),
            'p5': nn.Conv2d(self.out_channels, self.out_channels,kernel_size=3, stride=1, padding=1)
        })
    ...
    def forward(self, images: torch.Tensor):
        ...
        ## This part is to fulfill paper's Figure2 
        ...
        C3_ = self.fpn_params['l3'](backbone_feats['c3'])
        C4_ = self.fpn_params['l4'](backbone_feats['c4'])
        C5_ = self.fpn_params['l5'](backbone_feats['c5'])
        
        # Generate P5 from C5_
        P5 = C5_  # No upsampling, directly use C5_
    
        # Generate P4 from C4_ and downsample P5 to C4_ size
        P4 = C4_ + F.interpolate(P5, size=(C4_.shape[2], C4_.shape[3]), mode='nearest') 
    
        # Generate P3 from C3_ and sownsample P4 to C3_ size
        P3 = C3_ + F.interpolate(P4, size=(C3_.shape[2], C3_.shape[3]), mode='nearest') 
...
class FCOSPredictionNetwork(nn.Module):
    ...
    def __init__(
        ...
        for outputSize in stem_channels:
            # cls
            convLayer1 = nn.Conv2d(in_channels, outputSize,3,1,1)
            ReLU1 = nn.ReLU()
            nn.init.normal_(convLayer1.weight,mean=0,std= 0.01)
            nn.init.constant_(convLayer1.bias, 0)
            stem_cls.append(convLayer1)
            stem_cls.append(ReLU1)
            # box
            convLayer2 = nn.Conv2d(in_channels, outputSize,3,1,1)
            ReLU2 = nn.ReLU()
            nn.init.normal_(convLayer2.weight,mean=0,std= 0.01)
            nn.init.constant_(convLayer2.bias,0)
            stem_box.append(convLayer2)
            stem_box.append(ReLU2)
            in_channels = outputSize
    ...
    def forward(self, feats_per_fpn_level: TensorDict) -> List[TensorDict]:
        ...
        for fpn_level, feats in feats_per_fpn_level.items():
            class_logits[fpn_level] = self.pred_cls(self.stem_cls(feats))
            B = class_logits[fpn_level].shape[0]
            C = class_logits[fpn_level].shape[1]
            class_logits[fpn_level] = class_logits[fpn_level].view(B, C, -1).permute(0, 2, 1)      #view (H,W,C) -> (H*W, C)
            
            boxreg_deltas[fpn_level] = self.pred_box(self.stem_box(feats))
            boxreg_deltas[fpn_level] = boxreg_deltas[fpn_level].view(B, 4, -1).permute(0, 2, 1)

            centerness_logits[fpn_level] = self.pred_ctr(self.stem_box(feats))
            centerness_logits[fpn_level] = centerness_logits[fpn_level].view(B, 1, -1).permute(0, 2, 1)
...
class FCOS(nn.Module):
    ...
    def forward(
        ...
        backbone_feats = self.backbone(images)
        #print("Backbone feature shapes:", {k: v.shape for k, v in backbone_feats.items()})
        #print(f"self.pred_net(backbone_feats{self.pred_net(backbone_feats)}")

        pred_cls_logits, pred_boxreg_deltas, pred_ctr_logits = self.pred_net(backbone_feats)
        #print("Class logits:", pred_cls_logits)
        #print("Box regression deltas:", pred_boxreg_deltas)
        #print("Centerness logits:", pred_ctr_logits)
        ...
        fpn_shape = {"p3":backbone_feats["p3"].shape, "p4":backbone_feats["p4"].shape, "p5":backbone_feats["p5"].shape}
        locations_per_fpn_level = get_fpn_location_coords(fpn_shape,self.backbone.fpn_strides, device=images.device)
        #print("Locations per FPN level:", locations_per_fpn_level)
        ...
        for i in range(images.shape[0]):
            ...
            #print(f"Matched GT boxes for image {i}:", matched)
        ...
        #print(type(matched_gt_boxes))

        ### For classification
        ground_truth_classes = matched_gt_boxes[:,:,4].clone()
        pixel = ground_truth_classes==-1
        bg_mask = pixel
        ground_truth_classes[bg_mask] = 0
        ground_truth_classes_one_hot = torch.nn.functional.one_hot(ground_truth_classes.long(),self.num_classes)
        ground_truth_classes_one_hot = ground_truth_classes_one_hot.to(gt_boxes.dtype)
        ground_truth_classes_one_hot[bg_mask] = 0
        loss_cls = sigmoid_focal_loss(inputs=pred_cls_logits, targets=ground_truth_classes_one_hot)
        ...
        loss_box = generalized_box_iou_loss(pred_boxes.reshape(-1,4), matched_gt_boxes[:,:,:4].reshape(-1,4),reduction="none")
