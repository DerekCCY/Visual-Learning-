...
class Trainer(object):
    ...
    def loss(self, predictions, labels):
        ...
        # Mask where labels are not equal to the null token (e.g., self._null)
        mask = labels != self.model._null  # Shape: (N, T)
        
        # Flatten the predictions and labels, ignoring the null tokens
        predictions_flat = predictions.reshape(-1, predictions.size(-1))  # Shape: (N*T, V)
        labels_flat = labels.reshape(-1)  # Shape: (N*T)
        
        # Mask out the null token positions
        mask_flat = mask.reshape(-1)  # Shape: (N*T)
        
        # Cross-entropy loss
        loss_fn = nn.CrossEntropyLoss(reduction='none')  # Don't average the loss yet
        loss = loss_fn(predictions_flat, labels_flat)  # Shape: (N*T)
        
        # Apply the mask, and only calculate loss for non-null tokens
        loss = loss * mask_flat.float()  # Shape: (N*T), masked loss
        
        # Compute the average loss over the batch and sequence
        loss = loss.sum() / mask_flat.sum()  # Sum the loss and divide by the number of valid tokens
