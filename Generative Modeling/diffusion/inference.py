...
def get_fid(gen, dataset_name, dataset_resolution, z_dimension, batch_size, num_gen):
    ...
    def gen_fn(z_batch):
        # Assume z_batch is a tensor of shape (batch_size, z_dimension)
        img_shape = (z_batch.size(0), gen.channels, dataset_resolution, dataset_resolution)
        batch_samples = gen.sample(img_shape)
        # Scale the images to [0, 255]
        #print(f'min: {batch_samples.min()}')
        #print(f'max: {batch_samples.max()}')
        batch_samples = (batch_samples * 255).clamp(0, 255).to(torch.uint8)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--ddim-eta', type=int, default=0.1, help="Eta for DDIM")
