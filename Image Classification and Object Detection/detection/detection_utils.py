...
def fcos_make_centerness_targets(deltas: torch.Tensor):
    ...
    # Mask for background boxes
    background_mask = (deltas == -1).all(dim=1)

    # Compute centerness for non-background boxes
    left = deltas[:, 0]
    top = deltas[:, 1]
    right = deltas[:, 2]
    bottom = deltas[:, 3]

    # Calculate min and max for centerness computation
    min_left_right = torch.min(left, right)
    min_top_bottom = torch.min(top, bottom)
    max_left_right = torch.max(left, right)
    max_top_bottom = torch.max(top, bottom)

    # Calculate centerness using the provided formula
    centerness[~background_mask] = torch.sqrt(
        (min_left_right[~background_mask] * min_top_bottom[~background_mask]) /
        (max_left_right[~background_mask] * max_top_bottom[~background_mask])
    )
