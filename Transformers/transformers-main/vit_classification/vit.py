...
sys.path.append("/home/ccy/CCY/transformers-main") 
...
class ViT(nn.Module):
    ...
    def __init__(self, patch_dim, d_model, d_ff, num_heads, num_layers, num_patches, num_classes, device = 'cuda'):
        ...
        self.positional_encoding = PositionalEncoding(d_model,dropout=0.1) # TODO (use the positional encoding from the transformer captioning solution)
    ...
    def patchify(self, images):
        ...
        patches = patches.contiguous().view(N,-1,self.patch_dim*self.patch_dim*C)
    ...
    def forward(self, images):
        ...
        patches_embedded_cls = torch.cat((cls_tokens, patches_embedded), dim=1) # TODO (append a CLS token to the beginning of the sequence of patch embeddings)

        output = self.positional_encoding(patches_embedded_cls)
        mask = torch.ones((self.num_patches+1, self.num_patches+1), device=self.device)

        for layer in self.layers:
            output = layer(output, mask)

        cls_embedding = output[:, 0] # TODO (take the embedding corresponding to the [CLS] token and feed it through a linear layer to obtain the logits for each class)
