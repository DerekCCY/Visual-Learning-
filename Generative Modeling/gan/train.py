...
def get_optimizers_and_schedulers(gen, disc):
    ...
    #scheduler_generator = torch.optim.lr.scheduler.StepLR(optim_generator, stepsize=100000, gamma=0.1)
...
def train_model(
    ...
    train_loader = torch.utils.data.DataLoader(
        Dataset(root="/home/ccy/CCY/datasets/CUB_200_2011_32", transform=ds_transforms),
    ...
    while iters < num_iterations:
        for train_batch in train_loader:
            with torch.cuda.amp.autocast(enabled=amp_enabled):
                ...
                # Generator output
                #latent_vectors = torch.randn(batch_size, 128, device=train_batch.device)
                fake_batch = gen(batch_size)
                # Discriminator output on the train batch
                discrim_real = disc(train_batch)
                # Discriminator output on the generated data
                ...
                # THis part is for 'gradient penalty'
                ###################################################################
                
                alpha = torch.rand(batch_size, 1, 1, 1, device=train_batch.device)
            ...
            if iters % log_period == 0 and iters != 0:
                with torch.no_grad():
                    with torch.cuda.amp.autocast(enabled=amp_enabled):
                        ...
                        # Ensure generated samples are in the range [0, 1]
