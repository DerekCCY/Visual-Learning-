...
'''Generator network - Increase spatial resolution'''
...
class UpSampleConv2D(torch.jit.ScriptModule):
    def __init__(
        ...
        self.conv = nn.Conv2d(
            input_channels, n_filters, kernel_size=kernel_size, stride=stride, padding=padding
        )
        self.upscale_factor = upscale_factor
        # Instantiate PixelShuffle in __init__
    ...
    def forward(self, x):
        ...
        ##################################################################
        # 1. Repeat x channel-wise upscale_factor^2 times
        x = x.repeat(1, int(self.upscale_factor ** 2), 1, 1)  # Replicate values in the channel dimension
        #print(f"Input shape: {x.shape}")

        # 2. Apply PixelShuffle
        x = self.pixel_shuffle(x)  # Perform nearest neighbor upsampling
        #print(f"Output shape: {x.shape}")

        # 3. Apply convolution and return output
        output = self.conv(x)  # Apply convolution
        return output
        ##################################################################
        #                          END OF YOUR CODE                      #
        ##################################################################
...
'''Discriminator network - Reduce dimension and compress information'''

class DownSampleConv2D(torch.jit.ScriptModule):
    def __init__(
        self, input_channels, kernel_size=3, n_filters=128, downscale_ratio=2, stride =1, padding=0
    ):
        ...
        self.pixel_unshuffle = torch.nn.PixelUnshuffle(downscale_ratio)
    ...
    def forward(self, x):
        ...
        ##################################################################
        x = self.pixel_unshuffle(x)  # Unshuffle pixels
        B, C, H, W = x.shape
        x = x.view(int(self.downscale_ratio**2), B, C // int(self.downscale_ratio**2), H, W)
        x = x.mean(dim=0)  # Average across downscale factor
...
class ResBlockUp(torch.jit.ScriptModule):
    ...
    def __init__(self, input_channels, kernel_size=3, n_filters=128):
        ...
        self.layers = nn.Sequential(
            ...
            nn.ReLU(),
            UpSampleConv2D(input_channels=n_filters, kernel_size=3, n_filters=n_filters, stride=1, padding=1),
            nn.BatchNorm2d(n_filters, eps=1e-05, momentum=0.1, affine=True, track_running_stats=True)
        )
        self.upsample_residual = UpSampleConv2D(input_channels, kernel_size=(1,1), n_filters=n_filters, stride=1)
        ##################################################################
        #                          END OF YOUR CODE                      #
        ##################################################################
    ...
    def forward(self, x):
        ...
        # to the layer output.
        ##################################################################
        main_output = self.layers(x)            # Main branch
        residual_output = self.upsample_residual(x)  # Residual branch
        
        output = main_output + residual_output   # Add main and residual branches
        return output
        ##################################################################
        #                          END OF YOUR CODE                      #
        ############################################
...
class ResBlockDown(torch.jit.ScriptModule):
    ...
    def __init__(self, input_channels, kernel_size=3, n_filters=128):
        ...
        self.layers = nn.Sequential(
            ...
            nn.ReLU(),
            DownSampleConv2D(n_filters, kernel_size=kernel_size, stride=1, padding=1)
        )
        self.downsample_residual = DownSampleConv2D(input_channels, kernel_size=(1,1), stride=1)
        ##################################################################
        #                          END OF YOUR CODE                      #
        ##################################################################
    ...
    def forward(self, x):
        ...
        ##################################################################
        main_output = self.layers(x)                # Main branch
        residual_output = self.downsample_residual(x)  # Residual branch
        #print(f"main_output shape: {main_output.shape}, residual_output shape: {residual_output.shape}")
        output = main_output + residual_output       # Combine main and residual branches
        return output
        ##################################################################
        #                          END OF YOUR CODE                      #
        ##################################################################
...
class Generator(torch.jit.ScriptModule):
    ...
    def __init__(self, starting_image_size=4):
        ...
        ##################################################################
        self.dense = nn.Linear(in_features=128, out_features=2048, bias=True)     # This transformation prepares it for reshaping into a 4x4x128 feature map by reshaping the output into (B, 128, 4, 4)
        self.layers = nn.Sequential(
            ...
            nn.ReLU(),
            nn.Conv2d(128, 3, kernel_size=(3, 3), stride=(1, 1), padding=(1, 1)),    # turn into 3 channels
            nn.Tanh()
        )
        ##################################################################
        #                          END OF YOUR CODE                      #
    ...
    def forward(self, n_samples: int = 1024):
        ...
        # network.
        ##################################################################
        z = torch.randn(n_samples, 128, device=self.dense.weight.device)  # Shape: (n_samples, 128)
        images = self.forward_given_samples(z)
        return images
        ##################################################################
        #                          END OF YOUR CODE                      #
        ##################################################################
...
class Discriminator(torch.jit.ScriptModule):
    ...
    def forward(self, x):
        ...
        #x = x.mean(dim=(2, 3))  # Average across spatial dimensions if necessary
        return self.dense(x)
    
        # Summing across spatial dimensions ensures the network produces a single scalar value by aggregating the information in the feature maps.
        # This method is simple and effective, ensuring the network focuses on the overall information from the entire feature map.
