...
def softmax(logits):
    # Subtract the max logits for numerical stability
...
def cross_entropy_loss(outputs, targets):
    ...
    # Sum over the class dimension (1) and then average over the batch size
    loss = -torch.sum(targets * log_probabilities, dim=1).mean()
    return loss
