...
class AttentionLayer(nn.Module):
    ...
    def __init__(self, embed_dim, dropout=0.1):
        ...
        # nn.Linear layers are designed to work on the last dimension of the input tensor,
        # meaning they only apply their transformation to that final dimension, 
        # regardless of how many preceding dimensions there are.

        self.dropout = nn.Dropout(dropout)
            
    def forward(self, query, key, value, attn_mask=None):
        N, S, D = query.shape    # Batch size, Sequence length (query), Embedding dimension
        N, T, D = value.shape  # _, Sequence length (key/value), _
        ...
        # Q@K_Transpose / root(dk)
        scaling = D ** -0.5
        dot_product = torch.bmm(query, key.transpose(1,2)) * scaling
...
class MultiHeadAttentionLayer(AttentionLayer):
    ...
    def forward(self, query, key, value, attn_mask=None):
        ...
        query = self.query_proj(query).view(N, S, H, D_H).transpose(1, 2)  # Shape: (N, H, S, D/H)
        key = self.key_proj(key).view(N, T, H, D_H).transpose(1, 2)        # Shape: (N, H, T, D/H)
        value = self.value_proj(value).view(N, T, H, D_H).transpose(1, 2)  # Shape: (N, H, T, D/H)
        ...
        scaling = D_H ** -0.5
        dot_product = torch.matmul(query, key.transpose(-2, -1)) * scaling  # Shape: (N, H, S, T)

        if attn_mask is not None:
            # convert att_mask which is multiplicative, to an additive mask
            # Hint : If mask[i,j] = 0, we want softmax(QKT[i,j] + additive_mask[i,j]) to be 0
            # Think about what inputs make softmax 0.
            additive_mask = attn_mask.to(dot_product.dtype)
            additive_mask = additive_mask.masked_fill(additive_mask == 0, float('-inf'))
            dot_product += additive_mask
        
        # apply softmax, dropout, and use value
        # Softmax to get attention weights, then apply dropout
        attn_weights = F.softmax(dot_product, dim=-1)  # Shape: (N, H, S, T)
        attn_weights = self.dropout(attn_weights)

        # Weighted sum of values
        y = torch.matmul(attn_weights, value)  # Shape: (N, H, S, D/H)

        # concat embeddings from different heads, and project
        y = y.transpose(1, 2).contiguous().view(N, S, D)  # Shape: (N, S, D)
        output = self.head_proj(y)  # Final output projection to original embedding dimension
...
class PositionalEncoding(nn.Module):
    ...
    def forward(self, x):
        N, S, D = x.shape
        assert D == self.encoding.embedding_dim, "Embedding dimension must match"
        # TODO - add the encoding to x

        positions = torch.arange(S, device=device).unsqueeze(0).expand(N,S)
        pos_encodding = self.encoding(positions) #shape(N,S,D)
        
        output = x + pos_encodding
...
class SelfAttentionBlock(nn.Module):
    ...
    def forward(self, seq, mask):
        ...
        # Self-attention on the sequence with the mask
        attn_output = self.self_attn(seq, seq, seq, attn_mask=mask)  # Shape: (N, S, D)
        # Apply dropout to attention output
        attn_output = self.dropout(attn_output)
        # Add residual connection (self-attention output + original input)
        out = self.layernorm(seq + attn_output)  # Shape: (N, S, D)
...
class CrossAttentionBlock(nn.Module):
    ...
    def forward(self, seq, cond):
        ...
        attn_output = self.cross_attn(query=seq, key=cond, value=cond)
        attn_output = self.dropout(attn_output)
...
class FeedForwardBlock(nn.Module):
    ...
    def forward(self, seq):
        ...
        out = self.layernorm(seq + mlp_output)
...
class TransformerDecoder(nn.Module):
    ...
    def get_causal_mask(self, _len):
        ...
        mask = torch.tril(torch.ones((_len, _len), device=self.device))  # Lower triangular matrix
        #To prevent future positions from influencing the current prediction in the decoding step.
        return mask
